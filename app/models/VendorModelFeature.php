...
class VendorModelFeature extends BaseModel {

/*
CREATE TABLE IF NOT EXISTS `tbl_vendor_model_feature` (
  `_id` int(11) NOT NULL,
...
*/
	...
	protected $table = 'tbl_vendor_model_feature';
    ...
    public function models() {
        return $this->belongsToMany('VendorModel', 'tbl_vendor_model_model_feature', 'feature_id', 'model_id');
    }

    // Get Parent
    public function parent() {
        return VendorModelFeature::find($this->ft_group_id);
    }

    // Get Children
    public function children($onlyActive=true) {
        $children = VendorModelFeature::where( 'ft_group_id', $this->ft_group_id )->where('ft_level', '>', '1');
	...
	public static function search($pgn=1, $ppg=20, $params=array()) {
		...
		$model = new VendorModelFeature();

		$model = $model->where('ft_level', '1');

		if(isset($params['srchVMFName']) && trim($params['srchVMFName'])!='') {
			$model = $model->where('ft_label', 'like', '%' . $params['srchVMFName'] . '%');
		}
    ...
    public function savefeature($input) {
      ...
      try {
        ...
        $this->ft_fld_name = $this->getFeatureFieldName($this->ft_label, 'vmf_');
        ...
        foreach ($child as $k => $c) {
          if(isset($c['ft_label']) && !empty($c['ft_label'])) {
            $object = new VendorModelFeature($c);
            $object->populate($c);
            $object->ft_group_id = $this->ft_group_id;
            $object->ft_fld_name = $object->getFeatureFieldName($object->ft_label, 'vmf_');
